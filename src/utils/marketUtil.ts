...
export class MarketUtil {
  ...
  public static calculateFuelNeededForGood (ship: YourShip, good?: string): FuelTarget {
    const systemLocations = globals.locationMap.get((ship.location).substring(0,2));
    ...
    if (ship.cargo.length > 0 || !_.isEmpty(good)) {
        ...
        if (good) {
            ...
            goodToShip = ship.cargo.find((good) => { return (good.good !== "FUEL") })?.good;
        }
        if (goodToShip) {
            const systemGoods = globals.bestRoutesPerSystem.get(ship.location.substring(0,2));
            ...
            if (goodMarketData && goodMarketData.highLoc) {
                ...
                const currentLoc = systemLocations.find((loc) => { return loc.symbol === ship.location});
                if (targetDest && currentLoc && (targetDest.symbol !== currentLoc.symbol)) {
                    ...
                    fuelNeeded -= ship.cargo.find(item => item.good === 'FUEL')?.quantity || 0;
                    if (fuelNeeded < 0)
                        fuelNeeded = 0;
  ...
  private static backupNavigation(systemLocs: Location[], ship: YourShip): FuelTarget {
    const systemGoods = globals.bestRoutesPerSystem.get(ship.location.substring(0,2));
    let orderedMarket = _.orderBy(systemGoods, ['cdv'], ['desc']);
    let targetDest = systemLocs.find(loc => loc.symbol === orderedMarket[0].lowLoc && loc.symbol !== ship.location);
    if (!targetDest) {
  ...
  public static async updateMarketData(traders: IntraSystemTrader[]) {
    for (let [system, markets] of globals.universeMarkets) {
        await Promise.all(
            [...new Set(traders.filter(ship => ship.system === system && ship.ship.location !== undefined).map(ship => ship.ship.location))]
            .map((loc) => { return globals.spaceTraders.getMarketplace(loc) })
        ).then((data) => {
            ...
            for (const goods of market.marketplace) {
                if (goods.symbol !== 'FUEL') {
                    ...
                    if (item) {
                        ...
                        const newDist = MarketUtil.distance(markets.find(loc => loc.symbol === item.highLoc), markets.find(loc => loc.symbol === item.lowLoc));
