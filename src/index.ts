...
async function main() {
    globals.knownSystems.forEach(async (system) => {
        const locations = (await globals.spaceTraders.listLocations(system)).locations;
        globals.locationMap.set(system, locations.filter((loc) => loc.type !== 'WORMHOLE'));
    ...
    while(true) {
        try {
            ...
            await globals.spaceTraders.getShips().then(async (d) => {
                if(_.isEmpty(intraTraders)) {
                    for (const ship of d.ships) {
                        ...
                        if (ship.location) {
                          ...
                          trader.system = await (await globals.spaceTraders.getFlightPlan(ship.flightPlanId)).flightPlan.destination.substring(0,2);
                        }
                        intraTraders.push(trader);
                    }
                } else {
                    for (const ship of d.ships) {
                        let updateShip = intraTraders.find((cship) => { return cship.ship.id === ship.id});
                        let system: string;
                        if (ship.location) {
                            system = ship.location.substring(0,2);
                        } else {
                            const flight = await globals.spaceTraders.getFlightPlan(ship.flightPlanId);
                        ...
                        if (updateShip) {
                            ...
                            intraTraders.push(new IntraSystemTrader(ship, system));
                        }
                    }
                }
            ...
            (e) => {});
            await MarketUtil.updateMarketData(intraTraders);
        } catch (e) {
            console.log(e);
        }
        ...
        for (let purchaseLocation of ship.purchaseLocations) {
            for (let [system, shipsInSystemToBuy] of shipsToBuy) {
                if (
                    ...
                    shipsInSystemToBuy[ship.manufacturer][ship.class] > _.filter(intraTraders, (currShip) => {
                    ...
                    try {
                        await globals.spaceTraders.purchaseShip(purchaseLocation.location, ship.type);
